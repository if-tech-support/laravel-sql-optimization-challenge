...
**1 件ごとに追加のクエリ**が飛びます。注文 300 件なら「注文取得 1 本 + ユーザー取得 300 本 + 明細取得 300 本」と
...
2. 画面下部のバーの **`Queries` タブ**をクリックする。タブのカッコ内の数字が **クエリ本数**で、ここが **600 本超**になっているはず。クリックして開くと、`select * from users where id = ?` のような **同じ形の SQL が延々と並ぶ**のが見える ← これが N+1 の正体。
...
5. `Queries` タブの本数が **数本**まで激減していることを確認する（600 本超 → 数本）。画面の表示内容は変わらないこともあわせて確認する。

## 達成基準

- クエリ本数が **数本**（注文 + user + items）まで減っている。
- 画面表示は変わらない。
